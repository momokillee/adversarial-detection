"""Train the frequency-domain detector on clean vs combined adversarial images."""
...
class CleanAdvDataset(Dataset):
    ...
    def __init__(self, clean_dir: Path, adv_dirs, device: torch.device = torch.device("cpu")):
        """
        Initialize dataset.

        Args:
            clean_dir: Path to clean images
            adv_dirs: Iterable of directories containing adversarial images
        ...
        # Load adversarial images (label=1) from each directory
...
def count_images(directory: Path) -> int:
    if not directory.exists():
        return 0
    return sum(1 for p in directory.glob("*") if p.suffix.lower() in {".jpg", ".jpeg", ".png"})
...
def main():
    ...
    parser.add_argument("--gan-adv-dir", type=Path, default=Path("data/gan_adversarial"))
    parser.add_argument("--output", type=Path, default=Path("models/detector_gan.pt"))
    ...
    original_adv_count = count_images(args.adv_dir)
    gan_adv_count = count_images(args.gan_adv_dir)
    clean_count = count_images(args.clean_dir)

    print(f"Clean images: {clean_count}")
    print(f"Original adversarial images: {original_adv_count}")
    print(f"GAN adversarial images: {gan_adv_count}")
    ...
    dataset = CleanAdvDataset(args.clean_dir, [args.adv_dir, args.gan_adv_dir], device=device)
    if len(dataset) == 0:
        raise SystemExit("No images found. Add samples to data/clean/ and the adversarial directories.")
